	...
	<div class="app-container">
		...
		>
			...
			<el-button
				...
				icon="el-icon-search" style="margin-left:10px;padding: 7px 22px;" @click="getList"
			>
				查找
			</el-button>
			<el-button
				...
				@click="(listQuery.isCooperationOrisBlacklist = 2) && (listQuery.search = '') || getList()"
			>
				重置
			</el-button>
		</div>
		...
		<TableTools
			...
			@update-fields="updateFields" @refresh="getList" @download="toolsMixin_exportMethod($refs.vxeTable, '已拉黑师傅')"
		>
		</TableTools>
				...
				>
					<span v-if="row.workCity.split(',').length > 1">
						...
						<div
							...
							@click="$refs.DetailModal && $refs.DetailModal.handleOpen(row, 'region')"
						>
							更多
						</div>
					</span>
