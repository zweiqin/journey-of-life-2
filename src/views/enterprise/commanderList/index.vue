<template>
	<div class="app-container">
		...
		>
			<el-button
				v-permission="[ `POST ${api.savePartner}` ]" size="mini" type="success" icon="el-icon-plus"
				style="border: 0" @click="$refs.AddCommander && $refs.AddCommander.show()"
			>
				指定团长
			</el-button>
		</TableTools>
		...
		>
			...
			<template #sfUserWorkCity="{ row }">
				<span
					...
					style="position: relative;display: inline-block;height: 39px;overflow: hidden;text-overflow: ellipsis;word-break: break-all;white-space: normal !important;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;"
				>
					<span v-if="row.sfUserWorkCity.split(',').length > 1">
						<span>{{ row.sfUserWorkCity.match(/^[^,]*(?=,)/)[0] }}</span>
						<div
							style="position: absolute;top: 52%;right: 10px;width: fit-content;color: #6956e5;background-color: #ffffff;cursor: pointer;"
							@click="$refs.DetailModalWC && $refs.DetailModalWC.handleOpen(row, 'area')"
						>
							更多
						</div>
					</span>
			...
			<template #operate="{ row }">
				<el-button
					v-permission="[ `GET ${api.getPartnerInfo}` ]" :disabled="row.status < 3 || row.status == 5"
					size="mini" type="text" style="color: #2e8982"
				...
				<el-button
					:loading="isLoading" size="mini" :disabled="[3, 4].includes(row.status)"
					:type="[0, 1, 2].includes(row.status) ? 'warning' : row.status === 3 ? 'success' : row.status === 4 ? 'danger' : 'info'"
					style="color: #fff" @click="handleExcamine(row)"
				>
					{{ row.status | mapStatus }}
				</el-button>
				<el-button
					v-permission="[ `POST ${api.saveOrDeleteWorker}` ]" :disabled="row.status < 3 || row.status == 5"
					type="warning" size="mini" @click="
				...
				<el-button
					v-if="row.status === 3" v-permission="[ `DELETE ${api.deleteByPartner}` ]" type="danger" size="mini"
				...
				<el-button
					v-if="row.status === 5" v-permission="[ `DELETE ${api.deleteByPartner}` ]" type="primary" size="mini"
		...
		<!-- 指定团长 -->
		<ExamineDialog ref="ExamineDialog" @success="getList"></ExamineDialog>
	</div>
</template>
